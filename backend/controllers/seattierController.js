...
async function createTier(req, res) {
  try {
    ...
    const className = seat_class.trim();

    const [result] = await pool.query(
      'INSERT INTO seattier (seat_class, seat_type, created_at) VALUES (?, ?, NOW())',
      [className, seat_type ?? null]
    );
...
async function getTier(req, res) {
  try {
    ...
    console.error('getTier', err);
    return res.status(500).json({ message: 'Server error' });
  }
}

const getTierById = getTier;
