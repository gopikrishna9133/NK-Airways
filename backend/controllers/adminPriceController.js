...
async function listPrices(req, res) {
  try {
    ...
    console.error('adminPrice.listPrices', err);
...
async function getPrice(req, res) {
  try {
    ...
    console.error('adminPrice.getPrice', err);
    return res.status(500).json({ message: 'Server error' });
  }
}
...
async function createPrice(req, res) {
  try {
    ...
    try {
      ...
      if (uid) {
        const [r] = await pool.query('SELECT admin_id FROM admin WHERE user_id = ? LIMIT 1', [uid]);
        if (r && r[0]) adminId = r[0].admin_id;
      }
    } catch (e) {
      adminId = null;
    }
    ...
    console.error('adminPrice.createPrice', err);
    return res.status(500).json({ message: 'Server error' });
  }
}
...
async function updatePrice(req, res) {
  try {
    ...
    console.error('adminPrice.updatePrice', err);
    return res.status(500).json({ message: 'Server error' });
  }
}
...
async function deletePrice(req, res) {
  try {
    ...
    console.error('adminPrice.deletePrice', err);
    return res.status(500).json({ message: 'Server error' });
  }
}

module.exports = { listPrices, getPrice, createPrice, updatePrice, deletePrice };
